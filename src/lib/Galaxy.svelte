<script>
  ...
  const fragmentShader = `
    ...
    float Star(vec2 uv, float flare) {
      ...
      float rays = smoothstep(0.0, 1.0, 1.0 - abs(uv.x * uv.y * 3000.0));
      m += rays * flare * uGlowIntensity;
      uv *= MAT45;
      rays = smoothstep(0.0, 1.0, 1.0 - abs(uv.x * uv.y * 3000.0));
    ...
    vec3 StarLayer(vec2 uv) {
      ...
      for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
          ...
          if (Hash21(si * 12.34) > uDensity) continue;
  ...
  onMount(() => {
    ...
    // Attach mesh to program for easy access in update
  ...
  $: if (program) {
      ...
      program.uniforms.uScrollOffset.value = scrollOffset;
      program.uniforms.uBaseScale.value = baseScale;
  }
